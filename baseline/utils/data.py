...
def write_selection_preds(dataset_walker, output_file, data_infos, sorted_preds, topk=5):
    # Flatten the data_infos
    data_infos = [
        {
            "dialog_id": info["dialog_ids"][i],
    ...
    for info, sorted_pred in zip(data_infos, sorted_preds):
        dialog_id = info["dialog_id"]
        candidate_keys = info["candidate_keys"]

        snippets = []
        for pred_id, pred_score in sorted_pred[:topk]:
            selected_cand = candidate_keys[pred_id]
            domain, entity_id, doc_id = selected_cand.split("__")
            snippet = {
                "domain": domain,
                "entity_id": "*" if entity_id == "*" else entity_id,
                "doc_id": doc_id,
                "score": str(round(pred_score, 4))
            }
...
def make_input_masks(arrays, max_length=-1):
    if max_length < 0:
        max_length = max(list(map(len, arrays)))

    out = [
        [1] * len(array) + [0] * (max_length - len(array))
        for array in arrays
    ]
