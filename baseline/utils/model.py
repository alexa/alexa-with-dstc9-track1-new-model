...
def run_batch_selection_train(args, model, batch):
    ...
    input_ids, token_type_ids, input_masks, mc_token_ids, lm_labels, mc_labels = batch
    ...
    elif 'roberta' in args.model_for_ks:
        model_outputs = model(
            input_ids=input_ids, token_type_ids=None,
            attention_mask=input_masks, labels=mc_labels,
...
def run_batch_selection_eval(args, model, batch):
    ...
    batch = tuple(input_tensor.to(args.device) for input_tensor in batch if isinstance(input_tensor, torch.Tensor))
    input_ids, token_type_ids, input_masks, mc_token_ids, _, mc_labels = batch
    all_mc_logits = []
    for index in range(0, input_ids.size(1), candidates_per_forward):
        if 'gpt2' in args.model_for_ks:
            ...
            model_outputs = model(
                input_ids=input_ids[0, index:index + candidates_per_forward].unsqueeze(1),
                token_type_ids=None,
                attention_mask=input_masks[0, index:index + candidates_per_forward].unsqueeze(1),
            )
...
def run_batch_detection(args, model, batch):
    batch = tuple(input_tensor.to(args.device) for input_tensor in batch if isinstance(input_tensor, torch.Tensor))
    input_ids, token_type_ids, input_masks, mc_token_ids, lm_labels, labels = batch
    if 'gpt2' in args.model_for_ks:
        model_outputs = model(
            input_ids=input_ids, token_type_ids=token_type_ids,
...
def run_batch_classification(args, model, batch):
    batch = tuple(input_tensor.to(args.device) for input_tensor in batch if isinstance(input_tensor, torch.Tensor))
    input_ids, token_type_ids, input_masks, mc_token_ids, lm_labels, labels = batch
    if 'gpt2' in args.model_for_ks:
        model_outputs = model(
